...
Behaviour:
  ...
  It will write ty requirements under REPO_ROOT/resources/.
...
for pyv in 3.11 3.12 3.13 3.14; do
...
uv pip compile --python-version 3.14 --output-file resources/requirements_ty.txt "${upgrade_flag[@]}" resources/requirements_ty.in
