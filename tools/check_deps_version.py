...
def check_version_for_compatible_wheel(pkg_name: str, version: Version, required_py_ver: Version) -> bool:
    """
    ...
    if resp.status_code != 200:
        print(f"  Could not fetch data for {pkg_name} version {version} (status {resp.status_code}).")
        return False
    data = resp.json()
    files = data.get("urls", [])
...
def find_lowest_version_with_compatible_wheel(pkg_name: str, spec: SpecifierSet, pypi_data: dict,
                                              required_py_ver: Version, current_min: Version):
    """
...
def process_dependency(dep_str: str, required_py_ver: Version, origin: str):
    ...
    if specified_min is None:
        print(f"  {pkg_name} ({origin}) has no explicit lower-bound; skipping check.")
        return None

    print(f"  {pkg_name} ({origin}): specified minimal version = {specified_min}")

    if check_version_for_compatible_wheel(pkg_name, specified_min, required_py_ver):
        print(f"    OK: Version {specified_min} provides a compatible wheel for Python {required_py_ver}")
        return None
    else:
        ...
        resp = requests.get(url)
        if resp.status_code != 200:
            print(f"    Could not fetch data for {pkg_name} from PyPI (status {resp.status_code}).")
            return (pkg_name, specified_min, None)
        pypi_data = resp.json()
        alternative = find_lowest_version_with_compatible_wheel(pkg_name, spec, pypi_data, required_py_ver,
                                                                specified_min)
        if alternative:
            print(
...
def process_all_dependencies(dependencies, required_py_ver: Version, origin: str):
    """
...
def process_extras(extras: dict, required_py_ver: Version):
    ...
    for extra_name, dep_list in extras.items():
        ...
        issues.extend(process_all_dependencies(dep_list, required_py_ver, f"extra '{extra_name}'"))
    return issues
...
def main(pyproject_path="pyproject.toml"):
    ...
    if main_deps:
        ...
        overall_issues.extend(process_all_dependencies(main_deps, req_py_version, "main"))
    else:
    ...
    if overall_issues:
        print("\nSummary of problematic binary dependencies (for required Python version {}):".format(req_py_version))
