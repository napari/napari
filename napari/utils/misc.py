...
def move_indices(total, moving, insert):
    """Determine indices for a rearrangement event after a drag and drop.

    Assume the ```moving``` indices are dropped into the ```insert``` position,
    and that there ```total`` indices overall.
    ...
    moving : tuple
        Subset of indices that are moving.
    insert : int
        Location that indices are moving too.

    Returns
    -------
    tuple
        All indices ```0``` through ```total-1``` reordered to reflect the
        move.
    """
    index = moving[0]
