...
@pytest.mark.parametrize(
    ('canvas_size', 'expected_data_level'), ((1000, 0), (600, 1), (200, 2))
)
def test_on_draw_with_multiscale_image_canvas_size(
    make_napari_viewer, canvas_size, expected_data_level
):
    viewer = make_napari_viewer()
    qt_viewer = viewer.window._qt_viewer
    shapes = [(1600, 1600), (800, 800), (400, 400)]
    data = [np.empty(s) for s in shapes]
    layer = viewer.add_image(data, multiscale=True)
    qt_viewer.canvas.size = (canvas_size,) * 2
    # This does not get triggered automatically in the test.
    qt_viewer.on_resize(None)
    viewer.reset_view()
...
@pytest.mark.parametrize(
    ('camera_zoom', 'expected_data_level'), ((1, 0), (0.5, 1), (0.25, 2))
)
def test_on_draw_with_multiscale_image_camera_zoom(
    make_napari_viewer, camera_zoom, expected_data_level
):
    viewer = make_napari_viewer()
    qt_viewer = viewer.window._qt_viewer
    shapes = [(1600, 1600), (800, 800), (400, 400)]
    data = [np.empty(s) for s in shapes]
    layer = viewer.add_image(data, multiscale=True)
    qt_viewer.canvas.size = (1000, 1000)
    # This does not get triggered automatically in the test.
    qt_viewer.on_resize(None)
    viewer.reset_view()
