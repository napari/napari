...
class QtViewerDockWidget(QDockWidget):
    ...
    def destroyOnClose(self):
        ...
        self.ref_qt_viewer().viewer.window.remove_dock_widget(self)
    ...
    def keyPressEvent(self, event):
        ...
        return self.ref_qt_viewer().keyPressEvent(event)
    ...
    def _on_visibility_changed(self, visible):
        try:
            actions = [
                action.text()
                for action in self.ref_qt_viewer().viewer.window.plugins_menu.actions()
            ]
            idx = actions.index(self.name)

            current_action = (
                self.ref_qt_viewer().viewer.window.plugins_menu.actions()[idx]
            )
