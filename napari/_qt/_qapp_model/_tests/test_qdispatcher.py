...
def test_q_key_binding_dispatcher(qtbot):
    with patch.object(QKeyBindingDispatcher, 'executeCommand'):
        dispatcher = QKeyBindingDispatcher(
            CommandsRegistry(),
            check_repeatable_action=lambda x: x == 'repeatable',
        )
        mock: MagicMock = dispatcher.executeCommand
        ...
        # repeat same test but with conflicts
