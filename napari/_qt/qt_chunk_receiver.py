...
class QtChunkReceiver(QObject):
    """Receives newly loaded chunks from the ChunkLoader.
    ...
    We do not want to call into model/layer code in a worker thread
    so we signal ourselves in the GUI thread. If we are already in the GUI
    thread it does no harm.
    """
