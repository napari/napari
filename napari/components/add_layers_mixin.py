...
class AddLayersMixin:
    ...
    def add_path(
        ...
        def load(_path):
            ...
            if not layer_data:
                if isinstance(_path, (tuple, list)):
                    path_repr = f"[{_path[0]}, ...] as stack"
                else:
                    path_repr = _path
            ...
            for data in layer_data:
                layers = self._add_layer_from_data(*data)
                # some add_* methods return a List[Layer] others just a Layer
                added.extend(layers if isinstance(layers, list) else [layers])
