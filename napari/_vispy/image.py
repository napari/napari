...
uniform vec2 u_min_bounds;
uniform vec2 u_max_bounds;
varying vec2 v_texcoord;
...
void main()
{
    ...
    vec2 min_bounds_tex = u_min_bounds / image_size;
    vec2 max_bounds_tex = u_max_bounds / image_size;
    gl_FragColor = $color_transform($get_data(texcoord, min_bounds_tex, max_bounds_tex));
}
"""  # noqa

_interpolation_template = """
    #include "misc/spatial-filters.frag"
    vec4 texture_lookup_filtered(vec2 texcoord, vec2 min_bounds, vec2 max_bounds) {
...
_texture_lookup = """
    vec4 texture_lookup(vec2 texcoord, vec2 min_bounds, vec2 max_bounds) {
        if(texcoord.x < min_bounds.x || texcoord.x > max_bounds.x ||
        texcoord.y < min_bounds.y || texcoord.y > max_bounds.y) {
            discard;
        }
...
class ImageVisual(Visual):
    ...
    **kwargs : dict
    ...
    def set_data(self, image, bounding_box=None):
        """Set the data
        ...
        if self._bounding_box is None and bounding_box is None:
            ...
            self._bounding_box = bounding_box[::-1, :]

        self.shared_program['u_min_bounds'] = (
            self._bounding_box[0, 0],
            self._bounding_box[1, 0],
        )
        self.shared_program['u_max_bounds'] = (
            self._bounding_box[0, 1],
            self._bounding_box[1, 1],
        )
        self.shared_program['image_size'] = data.shape[:2][::-1]
        self._need_texture_upload = True
    ...
    def _bounding_box(self, bounding_box):
        if bounding_box is not None:
            ...
            self.shared_program['u_min_bounds'] = self._bounding_box_lims[:, 0]
            self.shared_program['u_max_bounds'] = self._bounding_box_lims[:, 1]
            self.update()
        else:
    ...
    def _initialize_bounding_box(self):
        """Initialize a bounding box to the full extent of the image texture"""
        # save to private variable to bypass self.update() call
        bounding_box_xlim = 0, self.size[0]
        bounding_box_ylim = 0, self.size[1]
