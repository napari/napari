...
import numpy as np
from vispy.color import Colormap as VispyColormap, ColorArray
...
class ImageLayerNode:
    def __init__(self, custom_node: Node = None):
        ...
        self._image_node = ImageNode(
            None, method='auto', texture_format='r32f'
        )
...
class VispyLabelsLayer(VispyBaseLayer):
    ...
    def _on_colormap_change(self, event=None):
        ...
        self.node.cmap = LabelColormap(
            colors=colormap.colors, controls=colormap.controls, seed=colormap.seed
        )
