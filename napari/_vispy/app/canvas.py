...
from vispy.util.event import EmitterGroup, Event, WarningEmitter
from vispy.util.ptime import time
from vispy.util.dpi import get_dpi
from vispy.util import config as util_config
from vispy.ext.six import string_types
from . import Application, use_app
from vispy.gloo.context import GLContext, set_current_canvas, forget_canvas
...
class Canvas(object):
    ...
    def create_native(self):
        ...
        if self._autoswap:
            self.events.draw.connect(
                (self, 'swap_buffers'), ref=True, position='last'
            )
    ...
    def _set_keys(self, keys):
        if keys is not None:
            if isinstance(keys, string_types):
                if keys != 'interactive':
                    raise ValueError(
                        'keys, if string, must be "interactive", '
                        'not %s' % (keys,)
                    )
        ...
        if len(keys) > 0:
            ...
            for key, val in keys.items():
                if isinstance(val, string_types):
                    ...
                    if new_val is None:
                        raise ValueError(
                            'value %s is not an attribute of ' 'Canvas' % val
                        )
                    val = new_val
    ...
    def connect(self, fun):
        ...
        try:
            ...
            raise ValueError(
                'Event "%s" not available on this canvas.' % eventname
            )
        # Connect
        emitter.connect(fun)
    ...
    @size.setter
    def size(self, size):
        return self._backend._vispy_set_size(
            size[0] * self._px_scale, size[1] * self._px_scale
        )
    ...
    def render(self):
        ...
        fbo = FrameBuffer(
            color=RenderBuffer(size[::-1]), depth=RenderBuffer(size[::-1])
        )
