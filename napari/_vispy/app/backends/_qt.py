...
from vispy.ext.six import text_type
...
def _check_imports(lib):
    ...
    for lib2 in libs:
        ...
        if lib2 in sys.modules:
            raise RuntimeError(
                "Refusing to import %s because %s is already "
                "imported." % (lib, lib2)
            )
...
def message_handler(*args):
    ...
    if qt_lib in ("pyqt4", "pyside"):
        msg_type, msg = args
    elif qt_lib in ("pyqt5", "qtpy"):
        msg_type, context, msg = args
    elif qt_lib:
        raise RuntimeError("Invalid value for qt_lib %r." % qt_lib)
    else:
        raise RuntimeError(
            "Module backends._qt ", "should not be imported directly."
        )

    if msg == (
        "QCocoaView handleTabletEvent: This tablet device is "
...
class QtBaseCanvasBackend(BaseCanvasBackend):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if p.fullscreen is not False:
            if p.fullscreen is not True:
                logger.warning(
                    'Cannot specify monitor number for Qt '
    ...
    def event(self, ev):
        ...
        # General touch event.
            ...
            self._vispy_canvas.events.touch(
                type='touch', pos=pos, last_pos=lpos
            )
        return out
...
class CanvasBackendEgl(QtBaseCanvasBackend, QWidget):
    ...
    def _vispy_set_current(self):
        egl.eglMakeCurrent(
            _EGL_DISPLAY, self._surface, self._surface, self._native_context
        )
...
class CanvasBackendDesktop(QtBaseCanvasBackend, QGLWidget):
    def _init_specific(self, p, kwargs):
        ...
        if p.context.shared.ref is widget:
            ...
            if 'shareWidget' in kwargs:
                raise RuntimeError(
                    'Cannot use vispy to share context and '
    ...
    def resizeGL(self, w, h):
        ...
        self._vispy_canvas.events.resize(
            size=(self.width(), self.height()), physical_size=(w, h)
        )
