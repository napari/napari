...
def test_add_complex_shape(shape_type, create_known_shapes_layer):
    ...
    for coord in desired_shape:
        ...
        event = ReadOnlyWrapper(
            CursorEvent(
                data_position=position, type='mouse_move', is_dragging=False
            )
        )
        mouse_move_callbacks(layer, event)
        event = ReadOnlyWrapper(
...
def test_drag_shape(create_known_shapes_layer):
    ...
    layer, n_shapes, _ = create_known_shapes_layer
    layer.scale_factor = 0.01  # zoom out so as to not select a vertex
    layer.mode = 'select'
    orig_data = layer.data[0].copy()
    assert len(layer.selected_data) == 0

    position = tuple(np.mean(layer.data[0], axis=0))
    # Simulate click to select layer
    event = ReadOnlyWrapper(
        CursorEvent(data_position=position, type='mouse_press')
    )
    mouse_press_callbacks(layer, event)
    ...
    # Simulate click to drag on now selected layer
    event = ReadOnlyWrapper(
        CursorEvent(
            data_position=position, type='mouse_press', is_dragging=True
        )
    )
    mouse_press_callbacks(layer, event)
    # start drag event
