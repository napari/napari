...
def test_rectangles_with_shape_type(ten_four_corner: list[np.ndarray]):
    ...
    assert np.all(
        [np.array_equal(ld, d) for ld, d in zip(layer.data, ten_four_corner, strict=False)]
    )
...
def test_3D_rectangles():
    ...
    assert np.all(
        [np.array_equal(ld, ld2) for ld, ld2 in zip(layer.data, layer2.data, strict=False)]
    )
    assert np.all([s == 'rectangle' for s in layer2.shape_type])
...
def test_ten_lines_with_shape_type(ten_two_corners):
    ...
    assert layer.nshapes == len(ten_two_corners)
    assert np.all(
        [np.array_equal(ld, d) for ld, d in zip(layer.data, ten_two_corners, strict=False)]
    )
    assert layer.ndim == ten_two_corners[0].shape[1]
    assert np.all([s == 'line' for s in layer.shape_type])
...
def test_copy_and_paste():
    ...
    assert np.all(
        [np.array_equal(a, b) for a, b in zip(layer.data[:2], layer.data[-2:], strict=False)]
    )
