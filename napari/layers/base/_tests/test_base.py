...
def test_sample_layer(unit_register):
    layer = SampleLayer(np.zeros((5, 10, 10)), 3)
    assert layer.ndim == 3
    assert layer.axis_labels == [f'dim_{i}' for i in range(3)][::-1]
...
def test_units(unit_register):
    layer = SampleLayer(
        np.zeros((10, 10)), 2, units='m', axis_labels=['x', 'y']
    )
...
def test_units_exceptions(unit_register):
    ...
    with pytest.raises(
        ValueError,
        match='If both axis_labels and units are provided.* Missing units for: y',
    ):
...
def test_axis_labels_exceptions():
    ...
    with pytest.raises(ValueError, match='Axes labels must be unique'):
        layer.axis_labels = ['x', 'x']
    with pytest.raises(
        ValueError,
        match=r'Length of axis_labels should be equal to ndim \(2\)',
    ):
        layer.axis_labels = ['x']
...
def test_set_axis_and_units(unit_register):
    ...
    mock1 = Mock()
    mock2 = Mock()
    layer.events.axis_labels.connect(mock1)
