...
class Layer(KeymapProvider, MousemapProvider, ABC):
    ...
    * `_basename()`: base/default name of the layer
    ...
    def thumbnail(self, thumbnail):
        ...
        # set background and padding values to match theme
        if theme == 'dark':
            # blend thumbnail with opaque black background
            background = np.zeros(self._thumbnail_shape, dtype=np.uint8)
            constant_values = 0
        elif theme == 'light':
            background = np.ones(self._thumbnail_shape, dtype=np.uint8) * 255
            constant_values = 255
        else:
        ...
        # thumbnails generated by _update_thumbnail may be truncated (limited
        # to extents of data), therefore, we pad values to ensure that the
        # final shape is what we expect
        ...
        # set opacity by blending background and actual thumbnail values
        # set background opaque
        background[..., 3] = 255
        # get the alpha values for the thumbnail as a percentage
        f_dest = thumbnail[..., 3][..., None] / 255
        # invert the alpha values as a percentage
        f_source = 1 - f_dest
