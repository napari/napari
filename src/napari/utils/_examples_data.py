...
class BackupDownloader(HTTPDownloader):
    def __call__(
        self, url: str, output_file: str, pooch: Any, check_only: bool = False
    ) -> bool | None:
        resource_id = DOI_TO_FILE_GOOGLE_ID[url]
        if check_only:
            download_url = GOOGLE_DRIVE_PREFIX + resource_id
            return super().__call__(
                download_url, output_file, pooch, check_only=check_only
            )
        gdown.download(id=resource_id, output=output_file, quiet=False)
        return None


def napari_choose_downloader(
    url: str, progressbar: bool = False
) -> HTTPDownloader | SFTPDownloader | FTPDownloader | DOIDownloader:
