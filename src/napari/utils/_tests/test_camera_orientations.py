...
def test_home_view_matches_base_directions(orientation):
    ...
    view, up = view_and_up_directions_from_angles((0, 0, 0), orientation)
    assert np.allclose(
        view,
...
def test_angles_from_view_direction_roundtrip(orientation, angles):
    ...
    view_direction, up_direction = view_and_up_directions_from_angles(
        angles, orientation
    )
    ...
    assert np.allclose(view_recovered, view_direction)
    assert np.allclose(up_recovered, up_direction)
...
def test_angles_from_view_direction_non_orthogonal_up(orientation):
    ...
    assert np.allclose(np.dot(up, view), 0)
