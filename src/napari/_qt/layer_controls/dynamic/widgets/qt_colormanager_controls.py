...
class QtColorManagerControl(QtWidgetControlsBase):
    ...
    def __init__(
        ...
        self.feature_label = QtWrappedLabel(f'{display_name} feature:')
        self.color_feature_combobox = QComboBox(parent)
        if self._layers[0]._edge.color_properties is not None:
            self.color_feature_combobox.setCurrentText(
    ...
    def change_color_mode(self, mode: ColorMode):
        with qt_signals_blocked(self.color_mode_combobox):
            for layer in self._layers:
                getattr(layer, self._colormanager_attribute).color_mode = mode
