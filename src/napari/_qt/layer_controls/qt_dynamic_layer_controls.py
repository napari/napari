...
class QtDynamicLayerControls(QtLayerControls):
    def __init__(self, layers) -> None:
        super().__init__()
        self._layers = layers 

        ####
        # Probably have to initialize stuff here? 
        ####

        #if len(self._layers) == 1:
            #logic for buttons here, if 1 layer take all buttons, otherwise only panzoom
        #else:
            #panzoom only
