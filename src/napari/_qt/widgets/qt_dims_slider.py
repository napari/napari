...
class QtDimSliderWidget(QWidget):
    ...
    def __init__(self, parent: QtDims, axis: int) -> None:
        ...
        # Unparented: a timer parented to the row can fire into it after deletion.
    ...
    def eventFilter(self, qobject: QObject, event: QEvent) -> bool:
        """Flash the padlock when a press lands on a control the lock disabled.

        Disabled controls do not reliably pass presses to the row (a disabled
        button swallows them), so the filter sits on each control. A press on
        the slider also makes this the active axis, as ``sliderPressed`` would.
        """
        if (
