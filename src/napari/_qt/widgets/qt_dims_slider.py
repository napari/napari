...
class QtPlayButton(QPushButton):
    ...
    def __init__(
        ...
        unit_combo.addItem(
            trans._('frames per second:'), PlaybackUnit.FRAMES_PER_SECOND
        )
        unit_combo.addItem(
            trans._('seconds per cycle:'), PlaybackUnit.SECONDS_PER_CYCLE
        )
        unit_combo.setToolTip(
            trans._(
                'Set playback speed as a frame rate, or as the duration of '
                'one full cycle through this axis.'
            )
        )
