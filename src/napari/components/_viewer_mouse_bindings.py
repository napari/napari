...
def drag_to_zoom(viewer, event):
    ...
    # on mouse press
    viewer.canvas._zoom_box.visible = True
    press_pos = event.pos[::-1]
    press_position = event.position
    move_pos = press_pos
    move_position = press_position
    viewer.canvas._zoom_box.position = (press_pos, press_pos)
    yield
    event.handled = True
    ...
    while event.type == 'mouse_move':
        if 'Alt' not in event.modifiers:
            viewer.canvas._zoom_box.visible = False
            yield
            return

        move_pos = event.pos[::-1]
        viewer.canvas._zoom_box.position = (press_pos, move_pos)
    ...
    if distance.min() > MIN_ZOOMBOX_SIZE:
        ...
        viewer.canvas._zoom_box.zoom_area = (
            press_position[-2:],
            move_position[-2:],
        )
    yield
