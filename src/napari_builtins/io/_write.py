...
CSV_EXTENSION = ".csv"
IMAGE_EXTENSIONS = (
    ".bmp", ".bsdf", ".bw", ".eps", ".gif", ".icns", ".ico", ".im",
    ".j2c", ".j2k", ".jfif", ".jp2", ".jpc", ".jpe", ".jpeg", ".jpf",
    ".jpg", ".jpx", ".lsm", ".mpo", ".npz", ".pbm", ".pcx", ".pgm",
    ".png", ".ppm", ".ps", ".rgb", ".rgba", ".sgi", ".stk", ".tga",
    ".tif", ".tiff"
)
...
def write_csv(
    ...
    try:
        ...
        raise IOError(f"Error writing to CSV file: {filename}") from e
...
def _write_image_data(path: str | Path, data: Any, meta: dict) -> Optional[Path]:
    """内部图像写入函数，处理多尺度数据和错误"""
    path = Path(path)
    ...
    if meta.get("multiscale", False):
        ...
        for i, level in enumerate(data):
            ...
            try:
                ...
                warnings.warn(f"Failed to write multiscale level {i}: {str(e)}")
    ...
    try:
        ...
        warnings.warn(f"Failed to write image: {str(e)}")
        return None
...
def _create_points_table(data: np.ndarray, properties: dict) -> Tuple[np.ndarray, list[str]]:
    """为点数据创建表格结构和列名"""
    ...
    if properties:
        ...
        for name, values in properties.items():
            if len(values) != n_points:
                warnings.warn(f"Property '{name}' has incorrect length, skipping")
                continue
...
def napari_write_points(path: str, data: Any, meta: dict) -> Optional[str]:
    ...
    try:
        ...
        warnings.warn(f"Failed to write points: {str(e)}")
        return None

def _create_shapes_table(data: list, shape_types: list) -> Tuple[np.ndarray, list[str]]:
    """为形状数据创建表格结构和列名"""
    # 确定最大维度
    ...
    # 构建表格行
    rows = []
    for shape_idx, (vertices, shape_type) in enumerate(zip(data, shape_types)):
...
def napari_write_shapes(path: str, data: Any, meta: dict) -> Optional[str]:
    ...
    try:
        ...
        warnings.warn(f"Failed to write shapes: {str(e)}")
        return None
...
def write_layer_data_with_plugins(
    ...
    try:
        ...
        raise IOError(f"Could not create directory {path}: {str(e)}") from e
        ...
        with TemporaryDirectory(dir=path) as tmp_dir:
            ...
            for layer in layer_data:
                ...
                try:
                    ...
                    warnings.warn(f"Failed to write layer '{layer_name}': {str(e)}")
