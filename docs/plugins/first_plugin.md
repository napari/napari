...
1. Our plugin contributes a [**command**](./contributions.html#contributions-commands)
   (we give the command an ID of `napari-hello.say_hi`. It must start with our plugin
   name, and be unique).
2. The location (**`python_name`**) of the function that executes the command. 
3. That our plugin contributes a widget, and that we'd like napari to **autogenerated**
   the widget from the command signature (so we don't need to deal with any GUI code)
