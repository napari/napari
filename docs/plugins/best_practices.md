...
Napari supports _both_ PyQt and PySide backends for Qt. It is up to the
...
Here's what can go wrong if you _also_ declare one of these backends in the
...
- Alternatively, they may end up with _both_ PyQt and PySide in their
...
- _What is a "wheel"?_
  ...
  Briefly, a wheel is a _built distribution_, containing code that is
...
- _How do I know if my dependency offers a wheel_
  ...
  _doesn't_ offer a wheel, it may still be ok if it's just a pure python package
  that doesn't have any C extensions...

- _How do I know if one of my dependencies uses C Extensions?_
...
_immediately_ when napari is launched, and we search the entry_point
...
manager to _uninstall_ your plugin, open file handles and resources may cause
...
order to test this, but you don't! Just _trust_ that napari will call this
