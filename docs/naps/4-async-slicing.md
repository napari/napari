...
Rather than exhaustively list all of the slice state of all layer types, we group and highlight
some of the more important state. For more details and context, see the
[slicing class and state dependency diagram](https://raw.githubusercontent.com/andy-sweet/napari-diagrams/main/napari-slicing-classes.drawio.svg).
...
These are typically just indexed as part of slicing.
For example, `Points.face_color` will be indexed by the points that are visible in the current slice.
...
the `slice_ready` event is emitted. That triggers `QtViewer._on_slice_ready` to be executed on
...
Connecting to the `slice_ready` signal may be sufficient, but that currently contains
a privately typed response, so may need more consideration.
