...
using the reference to the old data, though we may not want anything to consume
the output.
...
on the main thread.
That's useful because `QWidgets` and vispy nodes can only be safely updated
