...
plane_layer = viewer.add_image(
    ...
    blending='additive',
    opacity=0.5,
    slicing_plane=plane_parameters
)
...
def shift_plane_along_normal(viewer, event):
    ...
    intersection = plane_layer.slicing_plane.intersect_with_line(
    ...
    plane_normal_data_vispy = np.array(plane_layer.slicing_plane.normal)[[2, 1, 0]]
    plane_position_data_vispy = np.array(plane_layer.slicing_plane.position)[[2, 1, 0]]
    ...
    original_plane_position = plane_layer.slicing_plane.position
    ...
    while event.type == "mouse_move":
        ...
        updated_position = original_plane_position + drag_distance_data * np.array(
            plane_layer.slicing_plane.normal)

        if point_in_bounding_box(updated_position, plane_layer.extent.data):
            plane_layer.slicing_plane.position = updated_position
